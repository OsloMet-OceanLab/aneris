...
{
ws_t *wParam = static_cast<ws_t*>(param);
	...
	try
	{
		...
		{
			pArgs = PyTuple_New(1);
			PyTuple_SetItem(pArgs, 0, PyLong_FromLong(wParam->ws_port));
			PyTuple_SetItem(pArgs, 1, PyUnicode_FromString(wParam->ws_mode.c_str()));
